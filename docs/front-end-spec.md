...
**Implementation note:** Wrap the existing slot grid `div` in a new `div.visualizer-outer-shell` element. The outer shell provides the gray frame, handle tab (`:before` pseudo or inline child `div`), and wheel guards (two child `div`s at bottom). The `HousingUnitVisualizer` component's exported props interface is unchanged — the shell is interior to the component's root element.
