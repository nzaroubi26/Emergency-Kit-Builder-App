...
### Phase 2.6 Modified and New Files

| File | Action | Key Changes |
|------|--------|-------------|
| `src/components/cart/CartSidebar.tsx` | **New** | Persistent collapsible cart sidebar; derives all data from existing store fields (`itemSelections`, `selectedSubkits`, `emptyContainers`); open/close state owned by AppShell |
| `src/components/confirmation/OrderConfirmationScreen.tsx` | **New** | Post-"Get My Kit" confirmation screen; no store dependency after render; only outward action is Start Over → `resetKit()` + `navigate('/builder')` |
| `src/utils/cartCalculations.ts` | **New** | Pure functions: `calculateItemLineTotal`, `calculateSubkitCartTotal`, `calculateCartGrandTotal`; exports `CONTAINER_PRICES` constant; zero store dependencies |
| `tests/components/CartSidebar.test.tsx` | **New** | Component tests for CartSidebar |
| `tests/components/OrderConfirmationScreen.test.tsx` | **New** | Component tests for OrderConfirmationScreen |
| `tests/unit/cartCalculations.test.ts` | **New** | Unit tests for all three pure functions and `CONTAINER_PRICES` constant |
| `src/components/layout/AppShell.tsx` | Modified | Phase 2.6: mounts CartSidebar; owns `cartOpen` boolean local state; suppresses sidebar on `/confirmation` route via `useLocation()` |
| `src/components/layout/AppHeader.tsx` | Modified | Phase 2.6: adds ShoppingCart icon button with live item count badge (top-right) |
| `src/components/summary/SummaryScreen.tsx` | Modified | Phase 2.6: Get My Kit CTA calls `navigate('/confirmation')` instead of `initiateCheckout()` |
| `src/data/kitItems.ts` | Modified | Phase 2.6: `pricePlaceholder` populated for all 28 items |
...
export const router = createBrowserRouter([
  {
    ...
    children: [
      ...
      },
      {
        ...
        loader: confirmationGuard,  // redirects to /builder if selectedSubkits.length === 0
      },
      {
        path: '*',
        loader: () => redirect('/'),
...
export function confirmationGuard() {
  const { selectedSubkits } = useKitStore.getState();
  // Prevent direct URL access to /confirmation on a fresh session (no subkits selected)
...
├── [Get My Kit] → navigate('/confirmation')  [Phase 2.6 — replaces initiateCheckout()]
├── [Edit My Kit] → /builder
└── [Start Over] → ConfirmationModal → resetKit() → /builder

/confirmation (OrderConfirmationScreen)  [NEW — Phase 2.6]
├── Guard: confirmationGuard — redirects to /builder if selectedSubkits.length === 0
├── No store reads after initial render
├── CartSidebar is NOT rendered on this route (suppressed in AppShell via useLocation())
└── [Start Over] → resetKit() → navigate('/builder')
```
...
| 15 | **`CartSidebar` derives all displayed data from existing store fields only** — `itemSelections`, `selectedSubkits`, `emptyContainers`. It never dispatches store actions except `toggleItem` and `setItemQuantity`. It never stores open/closed state in Zustand — that state lives exclusively in `AppShell` local component state. | Prevents store bloat; keeps sidebar a pure read-with-limited-write consumer |
